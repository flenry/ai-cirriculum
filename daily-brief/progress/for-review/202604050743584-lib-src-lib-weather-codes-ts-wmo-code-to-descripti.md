...
**Moved to Review:** 2026-04-05 16:01:35
**PR:** _(no PR — direct commit)_

### What Was Done

Created src/lib/weather-codes.ts with WEATHER_CODES lookup map (WMO weather codes) and getWeatherDescription() function. Uses ?? operator for proper nullish coalescing (code 0 is falsy but valid). Maps 26 WMO codes to human-readable descriptions.

### How It Was Tested

Ran `npx vitest run tests/lib/weather-codes.test.ts` - all 4 tests pass: (1) code 0 returns 'Clear sky', (2) code 61 returns 'Slight rain', (3) code 999 returns 'Unknown (code: 999)', (4) code 100 returns 'Unknown (code: 100)'.
