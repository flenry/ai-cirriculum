...
**Moved to Review:** 2026-04-05 16:03:39
**PR:** _(no PR — direct commit)_

### What Was Done

Created src/api/github-api.ts with fetchGHNotifications() function. Fetches GitHub notifications with PAT auth, validates with zod schema, normalises to GHNotification[]. Uses normaliseGitHubUrl() for browser URL conversion. Returns empty array on error/auth failure with console.warn.

### How It Was Tested

Ran `npx vitest run tests/api/github-api.test.ts` - all 4 tests pass: (1) returns GHNotification[] on success, (2) returns [] when token empty, (3) returns [] on 401 response, (4) returns [] on Zod failure.
