...
**Moved to Review:** 2026-04-05 16:03:52
**PR:** _(no PR — direct commit)_

### What Was Done

Created src/api/wikipedia-api.ts with fetchWikiArticle() function. Fetches random Wikipedia article summary, validates with zod schema, returns WikiArticle. Truncates extract at word boundary if >280 chars. Returns null on error with console.warn.

### How It Was Tested

Ran `npx vitest run tests/api/wikipedia-api.test.ts` - all 5 tests pass: (1) returns WikiArticle on success, (2) does not truncate short extract, (3) truncates extract at word boundary, (4) returns null on HTTP 500, (5) thumbnailUrl is null when thumbnail absent.
